...
class Script(scripts.Script):
    def title(self):
        return "X/Y plot and run N times(change seed)"
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row():
            n_times_values = gr.Textbox(label="Run n times and fixed seed", visible=False, lines=1)
        
        draw_legend = gr.Checkbox(label='Draw legend', value=True)
        no_fixed_seeds = gr.Checkbox(label='Keep -1 for seeds', value=False)

        return [x_type, x_values, y_type, y_values,n_times_values, draw_legend, no_fixed_seeds]

    def run(self, p, x_type, x_values, y_type, y_values,n_times_values, draw_legend, no_fixed_seeds):
        ...
        print(f"X/Y plot will create {len(xs) * len(ys) * p.n_iter * len(nts)} images on a {len(xs)}x{len(ys)} grid. (Total steps to process: {total_steps * p.n_iter * len(nts)})")
        shared.total_tqdm.updateTotal(total_steps * p.n_iter * len(nts))
